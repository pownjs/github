...
class Github extends EventEmitter {
    ...
    async * repos(nickname, options) {
        // NOTE: this is not really that great way for detecting users vs orgs
    ...
    async * contributors(location, options) {
        const search = querystring.stringify({ ...options })
        ...
        for await (let items of this.paginate(`https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contributors?${search}`)) {
            for (let item of items) {
                yield item
